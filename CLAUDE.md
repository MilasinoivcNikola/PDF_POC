...
- @context/masterstories/story-9-master-template.md (Story 9 "[PET_NAME] and the New Baby" is the in-progress milestone — PR-A complete, PR-B next; remove this `@`-load on Story 9 completion)
...
- `context/masterstories/story-1-master-template.md`, `story-2-master-template.md`, `story-3-master-template.md`, `story-7-master-template.md`, `story-8-master-template.md`, `story-9-master-template.md` — the per-story source-of-truth wording (Stories 1–2 complete, Story 3 illustrative, Story 7 "Welcome Home" complete, Story 8 "Amazing Adventures" complete — Milestone 12; Story 9 "[PET_NAME] and the New Baby" in progress — Milestone 13, PR-A complete, PR-B next). Load when authoring or altering that story's text. **The in-progress story's masterstory stays `@`-loaded for the duration of its milestone** — add it to the always-loaded list above while building it, remove it on completion (Story 9 is `@`-loaded now; remove on its completion).
