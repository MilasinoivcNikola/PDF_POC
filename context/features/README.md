...
| 25 | [Story 6 — text, registration & living-tribute imagery](25-story6-text-registry-and-imagery.md) | 10 (Story 6) | 1 + 2 · `pdf-render-specialist` / `ai-image-specialist` | 14, 03, 04, 07 |
| 26 | [Story 6 — wizard, storefront & order intake](26-story6-wizard-and-storefront.md) | 10 (Story 6) | 3 · `nextjs-ui-builder` | 25 |
| 27 | [Lean the context system (history tiering · tests-as-truth · debt ledger)](27-lean-context-system.md) | — (dev-process) | — · main thread (verified by `context-auditor`) | — |
| 28 | Auto-wire the ledger into `/feature complete` (skill appends the `history.md` index line + `debt.md` rows) — *planned, spec TBD* | — (dev-process) | — · main thread | 27 |

> **Note:** Stories 2–3 were "out of scope" in the original `local-prototype-plan.md`. Story 2 (features 14–19) is a deliberate, PM-approved scope expansion (2026-06-09): full product, with the AI pet portrait (Premium tier), built on a generalized multi-story engine. Story 4 (features 20–22) is the first concrete title built via [`../new-book-playbook.md`](../new-book-playbook.md) (commerce PR-10) — "If [PET_NAME] Could Talk", the living/celebration twin of Story 2, split into 3 PRs. Story 5 (features 23–24) is the second playbook title — "A Letter to [PET_NAME]", the owner's-voice companion/inverse of Story 2, split into 2 PRs (its imagery is Story 2's exact shape, so the text and imagery fold into one PR). Story 6 (features 25–26) is the third playbook title — "While You're Still Here, [PET_NAME]", the **living tribute** (the catalog's only book made *before* a pet dies, for anticipatory grief), split into 2 PRs. It is the **first new book since Story 1 to reuse the narrative layout set** (`cover`/`dedication`/`narrative`/`love`/`back-cover`, not the letter treatment) and has **7 reference-anchored illustration slots** (Story-1-style reference anchor + scenes, not the letters' 2-image shape). The template's **memorial re-render / second-life path was dropped entirely** (PM decision 2026-06-12) — Story 6 ships as a pure living tribute with no commerce-loop touch.
...
- **M10 — Story 6 (While You're Still Here, [PET_NAME]):** 25 → 26. *Done = pick the living tribute on the landing page (or its storefront listing), walk the wizard / order form, generate the reference-anchored cover + dedication + 5 scene illustrations, preview the present-tense book as facing-page spreads, and download `While-Youre-Still-Here-[PET_NAME].pdf`.* The catalog's only **before-loss** keepsake (anticipatory grief); follows the new-book-playbook with **no new layout** — reuses Story 1's narrative set, with a Story-1-style reference anchor + 7 reference-anchored slots. The **memorial re-render is dropped** (PM, 2026-06-12). Stories 1 / 2 / 4 / 5 stay byte-identical.
