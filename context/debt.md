...
| `render:test` Story-1 filename quirk | `scripts/render-test.ts` | The CLI writes its local debug PDF with the Story-1 filename regardless of `storyType`; the production filename via `getStory().pdfFilename` is correct | low | Never for production; only if the debug filename matters |
...
| Playbook doc gaps | `context/new-book-playbook.md` | Followed in code but undocumented: every *sellable* book widens three `inputs` unions (`Order.inputs` / `OrderRow.inputs` / `AnySession`); the `tribute-*` page-id-prefix convention; the `slots + 1` reference-anchor image accounting | low | When the next book is added and an author misses one |
