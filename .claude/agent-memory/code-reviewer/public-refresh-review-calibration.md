...
**PR-3 verdict (/books two-section rewrite, `feature/public-refresh-catalog-page`) — CHANGES NEEDED, one blocker:**
- The blocker is the **count-line `countLabel`**. Spec + current-feature both mandate a `Record<string,{kicker;countLabel}>` lookup, and the mockup (`context/prototypes/001-public-pages-refresh/books.html`) frames **Story 1 as "12 pages"**, letters (story-2/5) as "2 illustrations". Impl dropped `countLabel` entirely (`CARD_COPY` only has `{kicker}`) and hardcodes `{illustrationCount} illustrations` for ALL books. Story-1 `illustrationCount` = **13** (cover + 12 pages via SCENE_PAGE_IDS) → renders **"13 illustrations"**, wrong on both axes (page-vs-illustration framing AND the off-by-one cover). "Derive the number, don't type it" is satisfied for the number but the per-book *unit word* half of the same decision is unmet.
- Nice-to-have: intro **lede** (spec checklist A) still describes only "a few titles" — was supposed to be rewritten to read for the whole 8-title line. Not addressed.
- **Validated as fine:** `.label--rose` is NOT a global util (only `.label--gold` exists in globals.css) — the page correctly scopes it via `.sectionHeadLoss :global(.label--rose)`; not the dead-CSS trap from PR-2. Placeholder paw block (Story 8/9) is graceful, tokens all resolve, no `"use client"`/dynamic markers (static tier held, boundary test green). `/books/${productId}` link target is unchanged from main and the `[productId]` detail route already exists on disk. products.ts `sampleImages:[]` + the test relaxation are sound and the tests stay meaningful (assert `[]` exactly + marketing copy still non-empty).

**Why (count-line):** "derive the number, don't type it" is easy to satisfy while still missing the per-book *label* half of the same decision.
**How to apply:** for any catalog-card count line, confirm BOTH the number is derived AND the unit word matches the mockup per book; don't accept a uniform "illustrations". Story-1's count is 13-not-12 because of the cover slot.
